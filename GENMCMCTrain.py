...
#Define parameters
Ntrain=100 #Number of training points to use
Ntest=20 #Number of test points to use
fasttime=1000
fastspin=100
...
#Create training points using MCMC (Use only data from the final iteration for the training and test data)
for k in range(5,10):
    ENKItrainind=np.empty((Ntrain))
    j=-1
    for i in range(0,Ntrain):
        while j<i:
            r=np.random.randint(1200,1600)
            if np.sum(ENKItrainind==r)==0:
                ENKItrainind[j]=r
                print(r)
                j+=1
    ENKItrainind=ENKItrainind.astype(int)
    ...
    for i in range(0,chains):
        samples[0,i,:]=XENKI[ENKItrainind[i],:]
    ...
    Xtrain=samples[ITER+burnin-1,:,:]





    Ytrain=np.zeros((Ntrain,N))
    ...
    for i in range(0,Ntrain):
        print(i)
        x0=np.random.rand(K*(J+1))
        Ytrain[i,:]=output(odeint(Lorenz96,x0,t,tuple(Xtrain[i,:])),fasttime,fastspin)
    

    np.save("/home/jprosser/Documents/WorkPlacements/Caltech/Data/MCMCXTrain{}.npy".format(k),Xtrain)
    np.save("/home/jprosser/Documents/WorkPlacements/Caltech/Data/MCMCYTrain{}.npy".format(k),Ytrain)
