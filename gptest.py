...
ITER=1600 #Number of times to run the Lorenz model
N=5
...
for i in range(0,ITER):
    ...
    z[0,i]=np.random.normal(loc=0.0,scale=1.0)
    z[1,i]=np.random.normal(loc=10.0,scale=10.0)
    z[2,i]=np.exp(np.random.normal(loc=2.0,scale=0.1))
    z[3,i]=np.random.normal(loc=5,scale=10.0)
...
np.save("parameters2.npy",np.transpose(z[:4,:]))
np.save("Output2.npy",np.transpose(z[4:,:]))
zt=np.transpose(z[4:,:])
diff=y_pred-zt
sqdiff=diff**2
print(np.sum(diff))
