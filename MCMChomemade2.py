...
def q(z):
    return np.random.multivariate_normal(z,(sigma**2)*np.identity(4))
...
def accept(zstar,z):
    A=np.min((1,st.norm.pdf(ztrue[M],loc=gp.predict(zstar.reshape((1,4)),return_std=True)[0],scale=gp.predict(zstar.reshape((1,4)),return_std=True)[1])/st.norm.pdf(ztrue[M],loc=gp.predict(z.reshape((1,4)),return_std=True)[0],scale=gp.predict(z.reshape((1,4)),return_std=True)[1])))
    test=np.random.rand()
    if A < test:
        return z
    else:
...
ztrue=np.load("/home/jonathan/Documents/WorkPlacements/Caltech/Data/ztrue.npy")
